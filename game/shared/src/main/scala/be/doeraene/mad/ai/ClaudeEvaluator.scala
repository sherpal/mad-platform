...
/** A hand-crafted [[GameState]] evaluator, built from a structural read of the ship stats rather than from the
  ...
  *
  * The starting observation: a capture only ever checks `attacker.attack >= defender.defence`. `movement` never
  * enters that decision, it's pure mobility. So the "true" combat tier of a piece is set by its `(attack, defence)`
  * pair, and it does not line up with the naming: 211 (a "frigate") has `attack=1, defence=1`, exactly as
  * combat-weak as 111 itself, while 122 (a "destroyer") has `attack=2, defence=2`, exactly as combat-strong as 222.
  * `movement` only earns a small secondary bonus.
  *
  * On top of that material scale:
  *   - a recall-aware correction: an exiled piece whose complementary piece is still on the board isn't fully dead,
  *     it can come back via a permutation (at the cost of a turn), except for 222, whose complement is 111 -
  *     "recalling" it would mean exiling your own 111, never a real option, so its exile is final, matching the
  *     rulebook ("once expelled, it can no longer be called back").
  ...
  *   - a phase-scaled 222 caution term: the cruiser is the strongest piece and its loss is permanent, but that
  *     matters most early, while many enemies remain to gang up on it; the caution fades as pieces trade off, so it
  *     doesn't just sit passively in the back row all game.
  *   - a hunt-pressure term: pulls our non-111 pieces toward the opponent's 111, so the evaluator has an actual plan
  *     to win rather than just a plan to not lose.
  *   - a small centre-control term, weighted a bit more for movement-2 pieces, which benefit more from it.
  *
  * Every numeric constant behind these terms lives in [[ClaudeWeights]] rather than here, so a tuner
  * (see [[be.doeraene.mad.ai.tuning.ClaudeWeightTuner]]) can search over them without touching this logic.
  *
...
object ClaudeEvaluator:
  ...
  /** Net combat score of an (attack, defence) pair: how many of the 8 opponent types this piece can take, minus how
    * many opponent types can take it. Only depends on (attack, defence): attack=2 can take all 8 types, attack=1
    * only the 4 with defence=1; defence=2 is only taken by the 4 attack=2 types, defence=1 is taken by all 8.
    */
  ...
  /** Material value of a piece still on the board. 111 gets none: its importance is entirely the terminal condition
    * and the safety term below, never a tradeable material amount.
    */
  ...
  /** Unlike [[GamePiece.piecesTakenScore]], which only fires once a capture is immediately available, this ramps up
    * a few squares out - a movement-2 piece starts contributing once within 4 squares, a movement-1 piece within 3 -
    * so a converging enemy shows up as a gradient the search can act on before it's a forced, too-late reaction.
    * This matters a lot at shallow depths: with only 3-4 plies of lookahead, a purely binary "am I in check right
    * now" term can leave the 111 boxed into a corner before any danger was ever visible to the search.
    */
  ...
  /** 111 has movement 1: it can never outrun a movement-2 attacker in open ground, only friendly pieces nearby can
    ...
    * every piece near the 111 as long as the 111 itself isn't in immediate danger yet - and then discovers, a couple
    * of moves later, that nothing is left close enough to help once a fast attacker does close in.
    */
  ...
  /** Without a term actively pulling pieces toward the opponent's 111, an evaluator built only from safety and
    * material tends to play soundly but passively: good at not losing, with no plan for winning, letting games drift
    * into thin endgames on the opponent's terms. This rewards each of our non-111 pieces (111 itself stays out of
    * it - it's too fragile to go hunting) for being few turns away from the opponent's 111, using the same
    * turns-to-travel [[GamePiece.turnToGoToOpponent111]] jPaul's theory is built on, tracking its *current* position
    * rather than a snapshot - reasonable here since this is recomputed fresh at every node the search visits anyway.
    *
    * Phase-scaled the other way from the cruiser caution above: early, with many enemies still able to counter-attack,
    * pulling escorts away from our own 111 to go hunting is exactly how it ends up undefended a few moves later, so
    * the pull starts weak; it strengthens as pieces thin out and the game increasingly comes down to a direct race.
    */
