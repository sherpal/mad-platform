...
/** A [[GameState]] evaluator built around the one thing a shallow minimax cannot see for itself: what is *about* to be
  ...
  *
  * So the centre of this evaluator is a single cheap board scan ([[Scan]]) that answers, for *every* ship at once,
  * "who can land on this square, and what happens if they do":
  *
  *   - [[Scorer.hangingScore]] resolves each threat as an exchange rather than a loss: a destroyer attacked by a ship
  *     that can be taken straight back costs the difference, not its whole value. This is what stops the evaluator
  *     from panicking about pieces that are perfectly well covered, and what lets it see that a trade it *can* take is
  *     good.
  *   - it is tempo-aware. Only one ship can be saved per turn and only one capture can be executed per turn, so the
  *     side to move discounts its own worst threat (it gets to answer it) and fully counts its opponent's. Without
  *     this the same position scores identically whoever is on move, which is exactly wrong in a game where almost
  *     every piece can be captured by something.
...
object TacticalEvaluator:
  ...
  /** Squares are indexed `row * 8 + col` into flat arrays rather than kept as `(Int, Int)` tuples in a `Map`. This
    * runs at every leaf of every search, and a tuple-keyed `Map` would box two `Int`s and hash them for every single
    * square lookup; both boards in play (6x4 and 5x5) have fewer than 8 columns, so the packed index is exact.
    */
  private inline val SquareCount = 64

  private val allPieces: Array[GamePiece] = Array.tabulate(GamePiece.pieces.size)(GamePiece.piecesByIndex)
  ...
  /** Per-piece constants that don't depend on the position, resolved once instead of at every node: the opaque
    ...
    */
  private val isCorvette: Array[Boolean]  = allPieces.map(_.is111)
  private val isRed: Array[Boolean]       = allPieces.map(_.team == Team.Red)
  private val isFast: Array[Boolean]      = allPieces.map(_.movement >= 2)
  private val movesOf: Array[List[GameAction.MovementAction]] =
    allPieces.map(GameAction.movementsByPiece.getOrElse(_, Nil))

  /** `canTake(a)(b)`: piece `a` may land on a square held by piece `b`, ie. they are enemies and `a` outguns `b`. */
  private val canTake: Array[Array[Boolean]] =
  ...
  private val complementOf: Array[Int] = allPieces.map(piece => indexOfPiece(GamePiece.oppositePieces(piece)))
  private val redCorvette: Int         = indexOfPiece(GamePiece.red111)
  private val blueCorvette: Int        = indexOfPiece(GamePiece.blue111)
  private val rotationPools: Array[Array[Int]] =
    GamePiece.rotationPools.map(_.toArray.map(indexOfPiece)).toArray
  ...
  private final class Scan(gameState: GameState):
    ...
    alive.foreach { index =>
      ...
      landings.foreach(square => reachersOf(square) ::= index)
    }
  ...
  /** Every square this ship could land on, ignoring who is standing there.
    ...
    * arrival square is takeable. Squares occupied by our own ships have to stay in this set, because the question
    * asked of it downstream is "could this ship recapture *after* the piece standing there is gone".
    */
  private def reachableSquares(gameState: GameState, piece: Int): List[Int] =
  ...
  final class Scorer(weights: TacticalWeights):
    ...
    /** Material value of a ship on the board. 111 is worth nothing here on purpose: it is never traded, its whole
      ...
      */
    private val valueOfPiece: Array[Double] = allPieces.map { piece =>
    ...
    /** What an exiled ship is still worth, expressed as what recalling it would actually *buy*.
      ...
      * alive, misreads the rule: a recall is never a gain of a ship, it is a *swap*. A permutation brings 122 back
      * only by exiling 211; a rotation brings a frigate back only by exiling one of the two still in the arena. So
      * the value of the whole recall option is the best single upgrade available - the exiled ship you'd want minus
      * the one you'd have to give up - and it's one option, not one per exiled ship, because it costs the turn either
      * way.
      *
      * 222 is the exception the rulebook itself calls out: its complement is 111, so "recalling" it means exiling
      * your own corvette and losing on the spot. Once exiled it is simply gone.
      */
    ...
    /** What the best capture against `piece` actually costs us, treating it as an exchange rather than a loss.
      *
      * A ship covered by a friend that can take the capturer straight back is not hanging; it is offering a trade,
      * and the trade only costs the difference in value. The attacker naturally picks whichever of its options nets
      * it the most, so this maxes over the attackers rather than averaging or taking the first.
      */
    ...
    /** Aggregates one side's hanging ships into a single number, respecting that a turn only buys one move.
      ...
      * those captures. So the worst threat counts fully and the next one only partially - and the side *on move*
      * flips that around, because it gets first say and can answer its own worst threat before it is ever executed.
      */
    ...
    /** How exposed this team's 111 is - in game-ending units, not material ones.
      ...
      *   - it is attacked right now. With the opponent on move that is simply the game; on our own move it is a
      *     threat we still have one turn to answer, hence [[TacticalWeights.corvetteTempoRelief]].
      *   - it has nowhere safe to go. Counting escape squares that are *covered by nothing* rather than merely legal
      *     is the whole point: a 111 with three legal moves onto three attacked squares is already lost, and an
      *     evaluator counting legal moves sees a comfortable position.
      *   - enemies are converging on it. A gradient rather than a binary, so a shallow search can steer away from
      *     danger several moves before it becomes a forced, too-late reaction - and, mirrored, so it can steer
      *     *toward* the enemy corvette.
      */
    ...
    /** Pulls our ships toward the enemy corvette, so the evaluator has a plan to win and not only a plan to not lose.
      *
      * Phase-scaled: early on, with a full enemy fleet still able to counter-attack, sending escorts off hunting is
      * how our own 111 ends up alone a few moves later, so the pull starts weak and strengthens as the board thins
      * out and the game turns into a direct race.
      */
    ...
    private def spaceScore(scan: Scan, teamIsRed: Boolean, centerRow: Double, centerCol: Double, maxDistance: Double)
        : Double =
      var mobility = 0.0
      var center   = 0.0
      scan.alive.foreach { piece =>
