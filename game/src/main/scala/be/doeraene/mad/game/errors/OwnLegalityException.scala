...
object OwnLegalityException:
  ...
  private class IllegalPiece(piece: GamePiece)
      extends OwnLegalityException(s"This piece ${piece.prettyPrint} is not valid.")
  private class NeutralPath(firstDirection: Direction, secondDirection: Direction)
      extends OwnLegalityException(
        s"This two directions are opposite of each other: $firstDirection and $secondDirection"
      )
  ...
  private class CompositeActionAreNotInSameTeam(actions: NatArray[GameAction])
      extends OwnLegalityException(s"These actions are not all for the same team: ${actions.mkString(", ")}")

  final class NotAllPathsLeadToRome(
      targetPositionOne: (Int, Int),
      targetPositionTwo: (Int, Int),
      paths: NatArray[(Direction, Direction)]
  ) extends OwnLegalityException(
        s"All the paths for this action don't lead to the same destination (found at least $targetPositionOne and $targetPositionTwo and these paths: ${paths
            .mkString(", ")})"
      )
  ...
  def compositionActionsNotAllInSameTeam(actions: NatArray[GameAction]): Option[OwnLegalityException] =
    Option.unless(actions.toVector.toList match {
      case Nil          => true
  ...
  def compositionActionsNotAllInSameTeam(actions: GameAction*): Option[OwnLegalityException] =
    compositionActionsNotAllInSameTeam(NatArray.from(actions))

  def shiftingActionDoesNotInvolvePiece(
      piece: GamePiece,
      action: GameAction.PieceShiftingAction
  ): Option[OwnLegalityException] =
  ...
  def notAllPathsLeadToRome(paths: NatArray[(Direction, Direction)]): Option[OwnLegalityException] =
    paths.map(_ + _).toVector.toList.distinct match {
      case Nil      => None
      case _ :: Nil => None
